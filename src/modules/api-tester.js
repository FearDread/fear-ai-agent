...
class APITester {
  ...
  async testEndpoint(args) {
    ...
    if (!url) {
      console.log('‚ùå Usage: test-endpoint <url> [method] [body]\n');
    ...
    console.log(`\nÌ†ΩÌ¥ç API Endpoint Security Test`);
  ...
  async testBasicSecurity(url, method) {
    console.log('Ì†ΩÌ≥ã Testing Basic Security...');
  ...
  async testHeaders(url) {
    console.log('Ì†ΩÌ¥ê Testing Security Headers...');
    ...
    try {
      ...
      console.log(`  ‚ö†Ô∏è  Header test failed: ${err.message}`);
    }
  }

  async testAuthentication(url, method) {
    console.log('Ì†ΩÌ¥ë Testing Authentication...');
    ...
    try {
      ...
      console.log(`  ‚ö†Ô∏è  Authentication test failed: ${err.message}`);
    }
  }
  ...
  async testRateLimiting(url, method) {
    ...
    try {
      ...
      console.log(`  ‚ö†Ô∏è  Rate limit test failed: ${err.message}`);
    }
  }

  async testInputValidation(url, method) {
    console.log('Ì†ΩÌª°Ô∏è  Testing Input Validation...');
  ...
  async testHTTPMethods(url) {
    console.log('Ì†ΩÌ¥ß Testing HTTP Methods...');
  ...
  displayResults() {
    ...
    if (critical.length > 0) {
      console.log('Ì†ΩÌ¥¥ CRITICAL Vulnerabilities:');
      critical.forEach(v => this.printVulnerability(v));
      console.log();
    }

    if (high.length > 0) {
      console.log('Ì†ΩÌø† HIGH Vulnerabilities:');
      high.forEach(v => this.printVulnerability(v));
      console.log();
    }

    if (medium.length > 0) {
      console.log('Ì†ΩÌø° MEDIUM Vulnerabilities:');
      medium.forEach(v => this.printVulnerability(v));
      console.log();
    }

    if (low.length > 0) {
      console.log('Ì†ΩÌø¢ LOW Vulnerabilities:');
    ...
    console.log(`Ì†ºÌæØ Security Score: ${score}/100`);
    console.log();
  }
  ...
  async testCollection(args) {
    ...
    if (!filePath) {
      console.log('‚ùå Usage: test-collection <json-file>\n');
    ...
    try {
      ...
      console.log(`\nÌ†ΩÌ¥ç Testing API Collection: ${collection.name}`);
      console.log(`‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê`);
      ...
      console.log(`‚ùå Failed to load collection: ${err.message}\n`);
    }
  }
