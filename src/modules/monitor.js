...
class TrafficMonitor {
  ...
  async startMonitoring(args) {
    if (this.monitoring) {
      console.log('‚ö†Ô∏è  Traffic monitoring is already running. Use "stop-monitor" first.\n');
      return;
    }

    const iface = args[0] || this.getDefaultInterface();
    
    console.log(`\nÌ†ΩÌ≥° Starting Traffic Monitor`);
    ...
    if (hasTcpdump) {
      ...
      console.log('‚ö†Ô∏è  tcpdump not found. Using fallback monitoring...\n');
  ...
  startTcpdumpMonitor(iface) {
    ...
    this.tcpdumpProcess.stderr.on('data', (data) => {
      ...
      if (msg.includes('Permission denied')) {
        console.log('‚ùå Permission denied. Try running with sudo.\n');
        this.stopMonitoring();
  ...
  async sampleNetworkActivity() {
    ...
    proc.on('close', () => {
      ...
      if (established > 0) {
        console.log(`Ì†ΩÌ≥ä Active connections: ${established}`);
      }
    });
  }
  ...
  detectSuspicious(line) {
    ...
    if (suspicious.length > 0) {
      suspicious.forEach(s => {
        ...
        console.log(`Ì†ΩÌ∫® ${s.type}: ${s.detail}`);
      });
    }
  }

  displayLiveStats() {
    console.log(`\nÌ†ΩÌ≥ä Live Stats (${this.stats.packets.total} packets)`);
  ...
  stopMonitoring() {
    if (!this.monitoring) {
      console.log('‚ÑπÔ∏è  Traffic monitoring is not running.\n');
      return;
    }
    ...
    this.monitoring = false;
    console.log('\n‚úÖ Traffic monitoring stopped.\n');
  }

  showStats() {
    if (!this.stats.startTime) {
      console.log('‚ÑπÔ∏è  No monitoring data available. Start monitoring first.\n');
      return;
    }
    ...
    console.log(`Status: ${this.monitoring ? 'Ì†ΩÌø¢ Active' : 'Ì†ΩÌ¥¥ Stopped'}\n`);
    ...
    if (this.stats.suspicious.length > 0) {
      console.log(`Ì†ΩÌ∫® Suspicious Activity (${this.stats.suspicious.length} events):`);
  ...
  async exportData(args) {
    ...
    try {
      ...
      console.log(`\n‚úÖ Traffic data exported to: ${filename}\n`);
    } catch (err) {
